...
def AddonMenu():  # homescreen
    ...
    if utils.has_upgraded():
        ...
        if _1CH.get_setting('show_splash')=='false':
            msg = ('None of the movies/episodes shown are hosted within this addon. The links are provided solely by the users.\n\n'
			'We cannot be held responsible for what users share. You may not use this addon to acess any material which you do not legally own and or have the rights to do so. It is your own responsibility to adhere to these terms.\n\n'
            '[I](This message is [B]only[/B] shown when the addon is first installed or updated.)[/I]')
            gui_utils.do_My_TextSplash(msg, HowLong=30, TxtColor='0xFF00FF00', BorderWidth=60)
        utils.TextBox()
...
def build_listitem(section_params, title, year, img, resurl, imdbnum='', season='', episode='', day='', extra_cms=None):
    ...
    # but only when item isn't in favs when using local favs 
        ...
        menu_items.append(('Add to Favourites', runstring), )
